...
class c_transaksiOther extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('m_transaksiOther');
	...
	{
		// Update status jadi 'belum' agar kembali ke menu Kualitas
		$this->db->where('id_kualitas', $id_kualitas);
		$this->db->where('id_penerimaan', $id_penerimaan);
		$this->db->update('tbl_kualitas', ['status' => 'kembali']); // ganti dengan nama tabel sebenarnya

		$this->session->set_flashdata('success', 'Data berhasil dikirim ulang ke Kualitas.');
		redirect('c_transaksiOther/index_report');
	}
	...
	{
		$data['supplier'] = $this->m_transaksiOther->indexSupplier()->result();
	...
	public function tambahaksi_supplier()
	{
		...
		$this->m_transaksiOther->insertSupplier($data,'m_supplier');

		redirect('c_transaksiOther/index_supplier');
	}

	public function edit_supplier($id_supplier)
	{
		$where = array('id_supplier' => $id_supplier);
		$data['edit'] = $this->m_transaksiOther->getByIdSupplier($where, 'm_supplier')->result();
	...
	public function editaksi_supplier()
	{
		...
		$this->m_transaksiOther->updateSupplier($id_supplier, $data);
		redirect('c_transaksiOther/index_supplier', $data);
	}
	...
	public function index_bcKualitas($report_no)
	{
		...
		if ($user_level == 1 || $user_level == 7) {
			$report_data = $this->m_transaksiOther->get_kualitas_reportno($report_no, 'belum');
			$report_type = 'kualitas';
		} elseif ($user_level == 10 || $user_level == 1) {
			$report_data = $this->m_transaksiOther->getReportSampleByReportNo($report_no, 'selesai');
		...
		$report_header = $this->m_transaksiOther->get_penerimaan_by_reportno($report_no);
	...
	public function login_adidas()
	{
		...
		public function index()
		{
			...
			$penerimaan_per_bulan = $this->m_transaksiOther->getPenerimaanPerBulan();
			$bulan = [];
			$jumlah = [];
			...
			$data['total_penerimaan'] = $this->m_transaksiOther->getTotalPenerimaan();
			$data['penerimaan'] = $this->m_transaksiOther->getPenerimaan()->result();
			$buyer = $this->m_transaksiOther->getBuyerChart();
			$timeline = $this->m_transaksiOther->getTimelineStatus();
			$labels = [];	
			$values = [];
			$timeline_labels = [];
			$timeline_values = [];
	...
	{
		//$data['hasil'] = $this->m_transaksiOther->get_pengujian_selesai();
		$data['hasil'] = $this->m_transaksiOther->get_pengujian_selesai();
		//$data['report_final'] = $this->m_transaksiOther->getReportFinal()->result();
		$this->template->load('layout/template','adidas/report/indexAll.php', $data);
	}

	public function index_report()
	{
		$data['report'] = $this->m_transaksiOther->getReportSample()->result();
	...
	public function hapus_penerimaan($id)
	{
		...
		if ($dipakai_di_handlingsample > 0 || $dipakai_di_kualitas > 0) {
			...
			redirect('c_transaksiOther/daftar_penerimaan');
			return;
		}
		...
		redirect('c_transaksiOther/index_penerimaan');
	}

	public function tambah_penerimaan()
	{
		$this->load->model('m_transaksiOther');
		$data['supplier'] = $this->m_transaksiOther->getSupplier()->result();
		$data['user'] = $this->m_login->current_user();
		$data['idPenerimaan'] = $this->m_transaksiOther->kodePenerimaan();
		$data['idKualitas'] = $this->m_transaksiOther->kodeKualitas();
		$data['department'] = $this->m_transaksiOther->getDepartment()->result();
		$data['email'] = $this->m_transaksiOther->getEmail()->result();
		$data['material'] = $this->m_transaksiOther->getMaterial()->result();
		$data['oekotex'] = $this->m_transaksiOther->getOekotex()->result();
		$data['stages'] = $this->m_transaksiOther->getStages()->result();
		$data['size'] = $this->m_transaksiOther->getSizecategory()->result();
		$data['washing'] = $this->m_transaksiOther->getCareWashing();
		$data['bleching'] = $this->m_transaksiOther->getCareBleching();
		$data['drying'] = $this->m_transaksiOther->getCareDrying();
		$data['profess'] = $this->m_transaksiOther->getCareProfess();
		$data['ironing'] = $this->m_transaksiOther->getCareIroning();
	...
	public function tambahaksi_penerimaan()
	{
		...
		foreach ($color as $i => $c) {
			...
			foreach ($test_required as $t) {
				$this->m_transaksiOther->saveKualitas(
					$t,
					$c,
					$co,
		...
		$penerimaan['id_penerimaan'] = $this->m_transaksiOther->kodePenerimaan();
		// Menyimpan data penerimaan ke database
		$this->m_transaksiOther->insertPenerimaan($penerimaan);
	...
	public function editaksi_penerimaan()
	{
		...
		$this->m_transaksiOther->updatePenerimaan($id_penerimaan, $penerimaan);
		...
		$this->m_transaksiOther->deleteKualitasByPenerimaan($id_penerimaan);
	...
	public function update_penerimaan()
	{
		...
		$this->m_transaksiOther->updatePenerimaan($id, $penerimaan);
		...
		if (is_array($colors) && is_array($tests)) {
			$this->m_transaksiOther->deleteKualitasByPenerimaan($id);
	...
	public function _generate_qrcode($report_no)
{
    ...
    $this->load->model('m_transaksiOther');
    $this->m_transaksiOther->insert_barcode_redirect($kode_unik, $report_no);
	...
	/*public function _generate_qrcode($report_no, $data_code)
	{
		...
		$url_redirect = $ip_server . '/handlingsample/index.php/c_transaksiOther/index_bcKualitas/' . urlencode($report_no);  // Menambahkan report_no ke URL
	...
	public function edit_penerimaan($id)
	{
		...
		$kualitas_data = $this->m_transaksiOther->getKualitasByPenerimaan($id);
		...
		$data['penerimaan'] = [
			'data'      => $this->m_transaksiOther->getByIdPenerimaan(['id_penerimaan' => $id], 'tbl_penerimaan')->row(),
			'kualitas'  => $kualitas_data,
			'email'     => $this->m_transaksiOther->getEmail()->result(),
			'material'  => $this->m_transaksiOther->getMaterial()->result(),
			'supplier'  => $this->m_transaksiOther->getSupplier()->result(),
			'oekotex'   => $this->m_transaksiOther->getOekotex()->result(),
			'stages'    => $this->m_transaksiOther->getStages()->result(),
			'size'      => $this->m_transaksiOther->getSizecategory()->result(),
			'washing'   => $this->m_transaksiOther->getCareWashing(),
			'bleching'  => $this->m_transaksiOther->getCareBleching(),
			'drying'    => $this->m_transaksiOther->getCareDrying(),
			'profess'   => $this->m_transaksiOther->getCareProfess(),
			'ironing'   => $this->m_transaksiOther->getCareIroning(),
			'image_path' => $image_path,
	...
	public function detail_kualitas($id_penerimaan, $id_kualitas)
	{
		...
		// Ambil semua data kualitas berdasarkan id_penerimaan
		$kualitas_data = $this->m_transaksiOther->getKualitasByPenerimaan($id_penerimaan);

		// Ambil data kualitas spesifik berdasarkan id_kualitas
		$kualitas_satu = $this->m_transaksiOther->getKualitasById($id_kualitas);
		...
		$data['penerimaan'] = [
			'data'          => $this->m_transaksiOther->getByIdPenerimaan(['id_penerimaan' => $id_penerimaan], 'tbl_penerimaan')->row(),
	...
	public function editaksi_report()
	{
		...
		$this->m_transaksiOther->editReport($id_reportkualitas, $id_handlingsample, $data);
		...
		if ($user_level == 10 || $user_level == 1) {
			redirect('c_transaksiOther/index_bcKualitas/' . url_title($this->input->post('report_no'), 'dash', TRUE));
		} else {
			redirect('c_transaksiOther/index_report');
		}
	}
	...
	{
		$data['detail'] = $this->m_transaksiOther->getKualitasRevisi($id_reportkualitas, $id_penerimaan)->row();
		$data['method'] = $this->m_transaksiOther->getReportMethod($id_reportkualitas);
    	$data['mode'] = 'edit';

		$data['kodemethod'] = $this->m_transaksiOther->kode_method();

		// Ambil data Method Groups
		$data['method_groups'] = $this->m_transaksiOther->get_method_groups();
		...
		if (!empty($this->input->post('method_group'))) {
			...
			$this->data['test_matrices'] = $this->m_transaksiOther->get_test_matrices_by_method_group($id_method_group);
		}
	...
	public function index_kualitas()
    {
        ...
        $data['kualitas'] = $this->m_transaksiOther->joinKualitas()->result();
		//$data['kualitas'] = $this->m_transaksiOther->joinKualitas_index()->result();
	...
	public function index_testResult($report_no = null, $id_kualitas = null, $id = null) {
		...
		if (empty($report_no) || empty($id_kualitas) || empty($id)) {
		 ...
		 // Untuk form create, method masih kosong atau ambil dari session/post jika pakai ajax
		$data['method'] = $this->session->userdata('keranjang_method') ?? []; // bisa kosong dulu
		$data['mode'] = 'create'; // optional, buat deteksi di view
		
	
		// Ambil data Method Groups
		$this->data['method_groups'] = $this->m_transaksiOther->get_method_groups();
		...
		$this->data['testmatrix'] = $this->m_transaksiOther->tampil_testmatrix()->result();
		$this->data['penerimaan'] = $this->m_transaksiOther->get_report($report_no);
		$this->data['id_kualitas'] = $id_kualitas;
		$this->data['kodemethod'] = $this->m_transaksiOther->kode_method();
		$this->data['testResult'] = $this->m_transaksiOther->get_test_result($report_no, $id_kualitas);
		$this->data['kodereport'] = $this->m_transaksiOther->kodeReportKualitas();
	
		// Ambil data penerimaan berdasarkan ID
		$where = array('id_kualitas' => $id);
		$this->data['penerimaan'] = $this->m_transaksiOther->getByIdKualitas($where, 'tbl_kualitas')->result();
	...
	/*public function index_testResult($report_no = null, $id_kualitas = null, $id = null, $id_reportkualitas = null, $id_handlingsample = null)
	{
		...
		? $this->m_transaksiOther->getReportMethod($id_reportkualitas) 
		: [];
		 	 
		if ($segment_count == 6) {
				$id_reportkualitas = $this->uri->segment(6);
				$existing_report = $this->m_transaksiOther->getTestRequired($id, $id_reportkualitas);
				$test_required_from_url = $existing_report->test_required ?? '';

				// Load test method jika report_kualitas sudah ada
				$this->data['testMethods'] = $this->m_transaksiOther->getAllTestMethodByReport($id_reportkualitas);
				
			} else {
				$test_required_from_url = $this->m_transaksiOther->getTestRequiredName($id_kualitas, $id);
		...
		if ($existing_report) {
			...
			if (!empty($existing_report->id_testmatrix)) {
				$testMatrix = $this->m_transaksiOther->getTestMatrixDetail($existing_report->id_testmatrix);
				if ($testMatrix) {
		...
		$this->data['testmatrix']   = $this->m_transaksiOther->tampil_testmatrix()->result();
		$this->data['report_data']  = $this->m_transaksiOther->get_report($report_no);
		$this->data['id_kualitas']  = $id_kualitas;
		$this->data['kodemethod']   = $this->m_transaksiOther->kode_method();
		$this->data['penerimaan']   = $this->m_transaksiOther->getByIdKualitas(['id_kualitas' => $id], 'tbl_kualitas')->result();
	...
	public function get_test_matrices_by_method_group($id_method_group) {
		$test_matrices = $this->m_transaksiOther->get_test_matrices_by_method_group($id_method_group);
	...
	public function get_all_method(){
		$data = $this->m_transaksiOther->lihat_nama_method($_POST['id_testmatrix']);
	...
	public function keranjang_method()
{
    $data_row = [
        ...
        'result_passfail'   => $this->input->post('result_passfail'),
        'result_passfail1'  => $this->input->post('result_passfail1'),
	...
	{
		$this->data['report'] = $this->m_transaksiOther->get_report_data_all($id_penerimaan);
		//$this->data['handling'] = $this->m_transaksiOther->get_report_data_adidas($id_penerimaan); 
		$this->data['method'] = $this->m_transaksiOther-> get_reportmethod_adidasAll($id_penerimaan);
	...
	public function report_test($id_handlingsample)
    {
		 ...
		 $this->data['handling'] = $this->m_transaksiOther->get_report_data_adidas($id_handlingsample); 
		...
		$this->data['deskripsi_item'] = $this->m_transaksiOther->get_deskripsi_item($id_handlingsample);
		...
		$this->data['method'] = $this->m_transaksiOther-> get_reportmethod_adidas($id_handlingsample);
	...
	public function tambahaksi_department()
	{
		...
		$this->m_transaksiOther->insertDepartment($data,'m_department');

		redirect('c_transaksiOther/index_department');
	}
	...
	public function tambahaksi_departmentOther()
	{
		...
		redirect('c_transaksiOther/index_departmentOther');
	}
	...
	public function edit_department($id_department)
	{
		...
		$data['edit'] = $this->m_transaksiOther->getByIdDepartment($where, 'm_department')->result();
	...
	public function editaksi_department()
	{
		...
		$this->m_transaksiOther->updateDepartment($id_department, $data);
		redirect('c_transaksiOther/index_department', $data);
	}
	...
	public function editaksi_departmentOther()
	{
		...
		$this->m_transaksiOther->updateDepartment($id_department, $data);
		redirect('c_transaksiOther/index_departmentOther', $data);
	}
	...
	{
		$data['department'] = $this->m_transaksiOther->joinDeptEmail()->result();
		$data['email'] = $this->m_transaksiOther->getEmail()->result();
	...
	public function tambahaksi_email()
	{
		...
		$this->m_transaksiOther->insertEmail($data, 'm_email');

		redirect('c_transaksiOther/index_email');
	}
	...
	public function tambahaksi_emailOther()
	{
		...
		redirect('c_transaksiOther/index_emailOther');
	}
	...
	public function edit_email($id)
	{
		$data['department'] = $this->m_transaksiOther->getDepartment()->result();
		$where = array('id' => $id);
		$data['edit'] = $this->m_transaksiOther->getByIdEmail($where, 'm_email')->result();
	...
	public function editaksi_email()
	{
		...
		$this->m_transaksiOther->updateEmail($id, $data);

		redirect ('c_transaksiOther/index_email', $data);
	}
	...
	public function editaksi_emailOther()
	{
		...
		redirect ('c_transaksiOther/index_emailOther', $data);
	}
	...
	public function getMaterialFill(){
		$data = $this->m_transaksiOther->materialFill($_POST['item_no']);
		echo json_encode($data);
	}

	public function getSupplierFill()
	{
		$data = $this->m_transaksiOther->supplierFill($_POST['supplier_name']);
		echo json_encode($data);
	}

	public function getApplicantFill(){
		$data = $this->m_transaksiOther->applicantFill($_POST['applicant']);
		echo json_encode($data);
	}

	public function index_material()
	{
		$data['index'] = $this->m_transaksiOther->getMaterial()->result();
	...
	public function tambahaksi_material()
	{
		...
		$this->m_transaksiOther->insertMaterial($data,'m_material');

		redirect('c_transaksiOther/index_material');
	}
	...
	public function tambahaksi_materialOther()
	{
		...
		redirect('c_transaksiOther/index_materialOther');
	}

	public function edit_material($id_material)
	{
		$where = array ('id_material' => $id_material);
		$data['edit'] = $this->m_transaksiOther->getByIdMaterial($where,'m_material')->result();
	...
	public function editaksi_material()
	{
		...
		$this->m_transaksiOther->updateMaterial($id_material, $data);
	...
	{
		$data['index'] = $this->m_transaksiOther->getBrand()->result();
	...
	public function tambahaksi_buyer()
	{
		...
		$this->m_transaksiOther->insertBrand($data, 'm_brand');

		redirect('c_transaksiOther/index_buyer');
	}
	...
	public function tambahaksi_buyerOther()
	{
		...
		redirect('c_transaksiOther/index_buyerOther');
	}

	public function edit_buyer($id_brand)
	{
		$where = array('id_brand' => $id_brand);
		$data['buyer'] = $this->m_transaksiOther->getByIdBrand($where, 'm_brand')->result();
	...
	public function editaksi_buyer()
	{
		...
		$this->m_transaksiOther->updateBrand($id_brand, $data);
		redirect('c_transaksiOther/index_buyer', $data);
	}
	...
	public function editaksi_buyerOther()
	{
		...
		$this->m_transaksiOther->updateBrand($id_brand, $data);
		redirect('c_transaksiOther/index_buyerOther', $data);
	}
	...
	{
		$data['index'] = $this->m_transaksiOther->getOekotex()->result();
	...
	public function tambahaksi_oekotex()
	{
		...
		$this->m_transaksiOther->insertOekotex($data, 'm_oekotex');

		redirect('c_transaksiOther/index_oekotex');
	}
	...
	public function tambahaksi_oekotexOther()
	{
		...
		redirect('c_transaksiOther/index_oekotexOther');
	}

	public function edit_oekotex($id)
	{
		$where = array('id' => $id);
		$data['edit'] = $this->m_transaksiOther->getByIdOekotex($where,'m_oekotex')->result();
	...
	public function editaksi_oekotex()
	{
		...
		$this->m_transaksiOther->udpateOekotex($id, $data);

		redirect('c_transaksiOther/index_oekotex', $data);
	}
	...
	public function editaksi_oekotexOther()
	{
		...
		redirect('c_transaksiOther/index_oekotexOther', $data);
	}
	...
	{
		$data['index'] = $this->m_transaksiOther->getSizecategory()->result();
	...
	public function tambahaksi_sizecategory()
	{
		...
		$this->m_transaksiOther->insertSizecategory($data,'m_age');

		redirect('c_transaksiOther/index_sizecategory');
	...
	public function tambahaksi_sizecategoryOther()
	{
		...
		redirect('c_transaksiOther/index_sizecategoryOther');
	...
	public function edit_sizecategory($id_age)
	{
		...
		$data['edit'] = $this->m_transaksiOther->getByIdSizecategory($where,'m_age')->result();
	...
	public function editaksi_sizecategory()
	{
		...
		$this->m_transaksiOther->updateSizecategory($id_age, $data);
	...
	{
		$data['index'] = $this->m_transaksiOther->getStages()->result();
	...
	public function tambahaksi_stages()
	{
		...
		$this->m_transaksiOther->insertStages($data, 'm_stages');
		redirect('c_transaksiOther/index_stages');
	}
	...
	public function tambahaksi_stagesOther()
	{
		...
		$this->m_transaksiOther->insertStages($data, 'm_stages');
		redirect('c_transaksiOther/index_stagesOther');
	}

	public function edit_stages($id_stages)
	{
		$where = array('id_stages' => $id_stages);
		$data['edit'] = $this->m_transaksiOther->getByIdStages($where,'m_stages')->result();
	...
	public function editaksi_stages()
	{
		...
		$this->m_transaksiOther->updateStages($id_stages, $data);
	...
	{
		$data['index'] = $this->m_transaksiOther->getCare()->result();
	...
	public function tambahaksi_care()
    {
        ...
        if (!$this->upload->do_upload('simbol_care')) {
            ...
            redirect('c_transaksiOther/index_careInstruction');
        } else {
            ...
            $insert = $this->m_transaksiOther->tambahCare($data);
	...
	public function tambahaksi_careOther()
    {
        ...
        if (!$this->upload->do_upload('simbol_care')) {
            ...
            $error = array('error' => $this->upload->display_errors());
            $this->session->set_flashdata('error', "Gagal mengunggah file: " . $error['error']);
            redirect('c_transaksiOther/index_careInstructionOther');
        } else {
            // Jika upload berhasil
            $upload_data = $this->upload->data();
            $file_name = $upload_data['file_name']; // Nama file yang diupload
    ...
    {   
        ...
        $this->load->library('upload', $config);
            ...
            if (!$this->upload->do_upload('simbol_care')){
                    ...
                    $insert = $this->m_transaksiOther->insertCare($name);
	...
	public function editaksi_care()
    {
        ...
        {
            $error = array('error' => $this->upload->display_errors());
            redirect('c_transaksiOther/index_careInstuction', $error);
        }else{
	...
	public function editaksi_careOther()
    {
        ...
        {
            $error = array('error' => $this->upload->display_errors());
            redirect('c_transaksiOther/index_careInstuctionOther', $error);
        }else{
	...
	public function hapus_kualitas($id_kualitas)
	{
		...
		redirect('c_transaksiOther/index_kualitas');
	}
	...
	public function hapus_care($id_care)
	{
		...
		redirect('c_transaksiOther/index_care');
	}
	...
	public function hapus_careOther($id_care)
	{
		...
		redirect('c_transaksiOther/index_careOther');
	}
	...
	public function tambahaksi_method()
	{
		...
		if (empty($testmatrix_post) || count(array_filter($testmatrix_post)) === 0) {
			...
			redirect('c_transaksiOther/index_testResult/' .
			$this->input->post('report_no') . '/' .
			$this->input->post('id_kualitas') . '/' .
			$this->input->post('id_penerimaan'));
			return;
		...
		$this->m_transaksiOther->insert_reportkualitas($data);

		// Insert ke report_handlingsample
		$this->m_transaksiOther->insert_handlingsample($data_report);
		...
		if ($this->db->trans_status() === FALSE) {
			...
			redirect('c_transaksiOther/error');
		} else {
...
public function editaksi_method()
{
    ...
    if (empty($testmatrix_post) || count(array_filter($testmatrix_post)) === 0) {
        $this->session->set_flashdata('error', 'Data metode pengujian tidak boleh kosong.');
        redirect('c_transaksiOther/edit_kualitas/' .
        $this->input->post('id_penerimaan') . '/' .
    ...
    for ($i = 0; $i < $jumlah_method; $i++) {
        array_push($data, ['id_testmatrix' => $this->input->post('id_testmatrix_hidden')[$i]]);
        $data[$i]['id_reportkualitas'] = $id_reportkualitas;
        $data[$i]['id_penerimaan'] = $this->input->post('id_penerimaan');
        $data[$i]['id_kualitas'] = $this->input->post('id_kualitas');
        $data[$i]['report_no'] = $this->input->post('report_no');
        $data[$i]['test_required'] = $this->input->post('test_required');
    ...
    // Hapus data lama di report_kualitas
    $this->m_transaksiOther->delete_reportkualitas_by_id($id_reportkualitas);

    // Insert ulang data ke report_kualitas
    $this->m_transaksiOther->insert_reportkualitas($data);

    // Update data di report_handlingsample
    $this->m_transaksiOther->update_handlingsample($data_report, $id_reportkualitas);

    // ✅ Update status di tbl_kualitas (tetap 'selesai')
    $this->db->where('id_kualitas', $this->input->post('id_kualitas'));
    $this->db->update('tbl_kualitas', ['status' => 'selesai']);
...
public function tambahaksi_methodbc()
{
	...
	if (empty($testmatrix_post) || count(array_filter($testmatrix_post)) === 0) {
		$this->session->set_flashdata('error', 'Data metode pengujian tidak boleh kosong.');
		redirect('c_transaksiOther/index_bcTestResult/' . 
			$this->input->post('report_no') . '/' . 
			$this->input->post('id_kualitas') . '/' . 
			$this->input->post('id_penerimaan'));
		return;
	...
	public function tambahaksi_methodOther()
	{
		...
		if ($this->db->trans_status() === FALSE) {
			...
			redirect('c_transaksiOther/index_kualitasOther');
		}
	}
	...
	{
		// Ambil data
		$data['detail'] = $this->m_transaksiOther->getReportUpdate($id_reportkualitas, $id_handlingsample)->row();
		$data['method'] = $this->m_transaksiOther->getReportMethod($id_reportkualitas);
	...
	{
		$data['user'] = $this->m_transaksiOther->tampil_data()->result();
        $data['level'] = $this->m_transaksiOther->get_level()->result();
    ...
    {
        $this->m_transaksiOther->delete_user($id_user);

        redirect('c_transaksiOther/index_user');
    }
	...
	public function hapusOther($id_user)
    {
        ...
        redirect('c_transaksiOther/index_userOther');
    }
	...
	public function tambah()
    {
        ...
        $this->m_transaksiOther->insert_user($data, 'tbl_login');
